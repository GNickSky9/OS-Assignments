/*
    ...
    ****UPDATED 10/14/20****
    ...
    And The Program Prints To Screen The Status Of The Threads And pIDs At
        Every Step.
    ****UPDATED 11/08/20****
    Included Mutex Locks In The threadWork Function That Each Thread Runs Upon
    Creation To Make Sure Only One Thread Can Ask To Allocate A pID At A Time
    To Avoid Having Two Threads Vying For The Same pID, Our Critical Section.
    Also Applies To When Releasing. C++ Standard Mutex Library Uses Pthreads
...
void *threadWork(threadInfo *info)
{
    ...
    temp.pID = allocate_pid();    // Critical Section.
    lock1.unlock();
    ...
    */
    ...
    lock1.lock();
    release_pid(temp.pID); // Critical Section potentially.
...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < TOTAL_PID; ++i) // Creating # Of Threads Equal To # Of pids.
    {
        ...
        /* Line Above (110) Not required but helps with output. */
